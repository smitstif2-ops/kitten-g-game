(function(KG){
  ...
  function spriteBottom(renderer,file){
    ...
    try{
      ...
      outer:for(let y=src.sh-1;y>=0;y--){
        for(let x=0;x<src.sw;x++)if(d[(y*src.sw+x)*4+3]>20){bottom=y;break outer;}
      }
  ...
  R.background=function(level,cameraX,time=performance.now()/1000){
    ...
    const colors=[['#d9edf2','#aacbd5'],['#def0f3','#a8cad3'],['#dcebe2','#aac7b8'],['#e5eef4','#b7cad8'],['#64545d','#292229']][level.background];
    ...
    if(im){
      ...
      c.save();
      c.globalAlpha=.94;
      c.filter=level.background===4?'saturate(.78) contrast(.88) brightness(.91)':'saturate(.68) contrast(.82) brightness(1.045)';
      c.drawImage(im,-(dw-this.w)/2-par,y,dw,dh);
      c.restore();
    }
    const haze=c.createLinearGradient(0,0,0,this.h);
    haze.addColorStop(0,level.background===4?'rgba(43,35,42,.08)':'rgba(239,247,249,.22)');
    haze.addColorStop(.68,level.background===4?'rgba(31,25,30,.035)':'rgba(235,244,247,.13)');
  ...
  R.backgroundAmbient=function(level,cameraX,time){
    ...
    if(level.background<4){
      c.strokeStyle='rgba(62,82,94,.22)';c.lineWidth=1.35;
      for(let i=0;i<4;i++){
    ...
    if(level.fx?.water||level.fx?.lava){
      const base=this.h*.72;c.globalAlpha=level.background===4?.11:.085;c.strokeStyle=level.background===4?'#ff9a58':'#fff';c.lineWidth=1;
      for(let i=0;i<6;i++){
  ...
  R.drawWaterBand=function(level,cameraX,time){
    ...
    const c=this.ctx,h=84,w=150,baseY=720;c.save();c.globalAlpha=level.background===3?.19:.25;c.globalCompositeOperation='screen';
  ...
  R.drawWaterfalls=function(level,cameraX,time){
    ...
    for(const [x,y,w,h] of arr){const c=this.ctx;c.save();c.globalCompositeOperation='screen';this.image(file,x-cameraX-w/2,y-h/2,w,h,false,.45,false);c.restore();}
  };
  ...
  R.drawTreeSway=function(level,cameraX,time){
    ...
    for(let i=0;i<arr.length;i++){
      ...
      c.save();c.translate(x-cameraX,feetY);c.rotate(angle);c.globalAlpha=.78;c.drawImage(src.im,src.sx,src.sy,src.sw,src.sh,-w/2,-h,w,h);c.restore();
    }
  };
  ...
  R.player=function(player,cameraX,time){
    ...
    const c=this.ctx;c.save();c.fillStyle='rgba(0,0,0,.20)';c.beginPath();c.ellipse(x,player.y+4,isDefault?30:28,7.5,0,0,Math.PI*2);c.fill();c.restore();
